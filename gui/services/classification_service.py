...
def svc_start_classification():
    global is_classifying
    is_classifying = True
    thread: threading.Thread = threading.Thread(target=_start_classification, daemon=True)
...
def _start_classification():
    global is_classifying
    classification: ClassificationPipeline = ClassificationPipeline()

    classification.classification_workflow()
    data_queue.put({"__done": True})
    is_classifying = False

def svc_stream_classification():
    while True:
        try:
            data = data_queue.get(timeout=30)
            yield f"data: { json.dumps(data) }\n\n"

            if data.get("__done") or not is_classifying:
                return

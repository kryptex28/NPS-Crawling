...
def svc_export_results(format: str):
    download_dir = Config.ROOT_DIR / "downloads"
    # ensure directory exists
    Path(download_dir).mkdir(parents=True, exist_ok=True)

    file_name = f"nps_filings_export.{format}"
    if format == "csv":
        _export_to_csv(filepath=download_dir / file_name, all=True)

    return download_dir / file_name

def _export_to_csv(filepath: Union[str, Path] = "nps_filings_export.csv", all: bool = True, keyword: str = "net promotor", strict: bool = False) -> None:
    """
    ...
    path = Path(filepath)
    print(f"Exporting data from '{db.table_name}' to '{path}'...")
    ...
    try:
        ...
        print(f"Successfully exported {len(rows)} rows to {os.path.abspath(str(path))}")
    except Exception as e:
